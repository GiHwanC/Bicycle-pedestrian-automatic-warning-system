...
| 이름            | 역할                        | 주요 기여 |
|----------------|-----------------------------|-----------|
| **김기환 (팀장)** | AI · 임베디드 통합 개발         | 프로젝트 전체 구조 설계, RealSense + YOLO + MTCNN 통합, 보행자 거리 계산 알고리즘 구현, 위험 판단 알고리즘 설계, Raspberry Pi GPIO(부저·램프) 제어 펌웨어, 테스트 시나리오 설계 및 개선 |
| 팀원           | 데이터 수집 & 실험 보조          | 자전거 주행 테스트, 촬영 환경 세팅, 라벨링 및 실험 보조 |
...
2. **거리 오판에 의한 대응 지연**  
   - 일반 RGB 영상만으로는 정확한 실제 거리 측정이 어려움  
...
1. 사람까지의 실제 거리 `≤ 6 m`  
2. MTCNN 얼굴 검출 실패 (보행자가 뒤를 보고 있거나, 시선이 반대 방향)

→ **부저(Buzzer)** : 0.5초 간격 토글  
→ **램프(Lamp)** : 2초간 점등  

### 5. Raspberry Pi GPIO 제어
- `BUZZER_PIN` / `LAMP_PIN` 을 통한 경고 장치 제어
- PWM/토글 기반 반복 경고
- `Ctrl + C` 종료 시 `GPIO.cleanup()` 수행으로 핀 상태 자동 정리
